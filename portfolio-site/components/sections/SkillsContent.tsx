  ...
  return (
    <div className="space-y-12">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {stats.map((stat, index) => (
          <motion.div
            ...
            className="p-6 rounded-xl bg-black/70 backdrop-blur-2xl border-2 text-center group relative"
            style={{
              borderColor: '#8b5cf660',
          ...
          >
            <motion.div 
              className="mb-2 bg-clip-text text-transparent bg-gradient-to-r from-blue-400 to-purple-400"
      ...
      <div className="grid md:grid-cols-3 gap-6">
        {skillCategories.map((category, categoryIndex) => (
          <motion.div
            ...
            className="p-6 rounded-2xl bg-black/70 backdrop-blur-2xl border-2 relative"
            style={{
              borderColor: '#8b5cf660',
              boxShadow: '0 0 30px rgba(139, 92, 246, 0.3), 0 0 60px rgba(139, 92, 246, 0.15)',
            }}
          >
            <h3 className="mb-6 text-white font-semibold">{category.category}</h3>
            <div className="space-y-5">
              {category.skills.map((skill, skillIndex) => (
                <div key={skill.name}>
                        <div className="flex justify-between items-center mb-2">
                          <span className="text-white text-sm font-medium">{skill.name}</span>
                          <span className="text-white/80 text-xs font-semibold">{skill.level}%</span>
                        </div>
      ...
      <motion.div
        ...
        className="text-center"
      >
        <p className="text-white/90 text-sm mb-6 font-medium">Technologies I work with</p>
        <div className="flex flex-wrap gap-3 justify-center">
          {[
            ...
            <motion.div
              ...
              className="px-4 py-2 rounded-full bg-black/60 backdrop-blur-xl border border-purple-500/40 text-white text-sm transition-all"
              style={{
